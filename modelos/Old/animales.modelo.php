...
class ModeloAnimales{
    ...
    $stmt = Conexion::conectar()->prepare("INSERT INTO $tabla(RFID,mmGrasa,peso,sexo,tas1,tas2,tas3,ecoRef,registroClas,date) VALUES(:rfid, :mmGrasa, :peso, :sexo, :tas1, :tas2, :tas3, :ecoRef,:registroClas,CURRENT_DATE)");
	...
	static public function mdlMostrarAnimales($tabla,$item,$valor){
      ...
      if($valor == null){
        ...
        $stmt = Conexion::conectar()->prepare("SELECT * FROM $tabla WHERE $item IS NULL ORDER BY date DESC");
    ...
    if($clas == 'gordas' OR $clas == 'flacas'){
      ...
      $stmt = Conexion::conectar()->prepare("SELECT COUNT(*) as total FROM $tabla WHERE $item $operador :$item AND $item3 = :$item3");
